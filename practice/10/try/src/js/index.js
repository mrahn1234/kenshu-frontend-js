$(document).ready(function () {
  var list = $(".list");
  var count = 1;
  var slide_width = parseInt($(".item").css("width"));
  var carousel_len = $(list).children().length; // length before prepend&append ,not prepend & append yet
  var first_clone = $(list.children()[0]).clone();
  var last_clone = $(list.children()[carousel_len - 1]).clone();
  first_clone.attr("id", "first-clone");
  last_clone.attr("id", "last-clone");
  list.append(first_clone);
  list.prepend(last_clone);
  var re_css = (step, transition = "transform 0.3s") => {
  ...
  var pause_carousel = false;
  var next_slide_time = 2000;
